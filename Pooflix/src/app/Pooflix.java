...
public class Pooflix {
    ...
    /**
     * Busca serie s en lista series y si hay coincidencia devuelve objeto tipo
     * serie
     * 
     * @param titulo
    ...
    public INominable estaNominadoo(String nombre) {

        
        for (INominable n : this.nominados) {
            if (n instanceof Actor) {
                Actor actorNominado = (Actor) n;        

                if (actorNominado.getNombre().equals(nombre)) {
                    actorNominado.estaNominada();                                     
                }
        ...
        for (INominable n : this.nominados){
             if (n instanceof Pelicula) {
                Pelicula peliculaNominada = (Pelicula) n;

                if (peliculaNominada.getNombre().equals(nombre)) {
                peliculaNominada.estaNominada();                                      
                }
    ...
    /**
     * INICIALIZA SERIE FRIENDS SE PODRIA PONER EN inicializarCatalogo pero lo
     * hacemos aparte para mantener orden.
     */
    public void inicializarFriends() {
        ...
        episodioReusandoVariable.setDescripcion(
                "Rachel Green entra a Central Perk usando un vestido de novia. Rachel les explica que dejó a su prometido en el altar.");
        ...
        episodioReusandoVariable
                .setDescripcion("Ross recibe la noticia de su exesposa Carol de que será padre del niño que espera.");
        temporadaReusandoVariable.getEpisodios().add(episodioReusandoVariable);

        episodioReusandoVariable = new Episodio(3, "The one with the thumb", 22);
        episodioReusandoVariable.setDescripcion(
                "Lizzy buys a soda for Phoebe, who, upon opening the can, discovers a severed thumb floating in the soda. The soda company gives Phoebe $7,000.");
        temporadaReusandoVariable.getEpisodios().add(episodioReusandoVariable);
        ...
        episodioReusandoVariable.setDescripcion(
                "Rachel sees Ross arrive with his new girlfriend, paleontologist and former graduate school classmate Julie.");
        temporadaReusandoVariable.getEpisodios().add(episodioReusandoVariable);
    ...
    /**
     * INICIALIZA SERIE THE OFFICE SE PODRIA PONER en inicializarCatalogo pero lo
     * hacemos aparte para mantener orden.
     */
    public void inicializarTheOffice() {

        // GENERO
        Genero mockumentary = new Genero("Mockumentary",
        ...
        // CLASIFICACION
        Clasificacion masDe13 = new Clasificacion("PG", 13,
        ...
        episodioTheOffice.setDescripcion(
                "Michael Scott, the regional manager at the Scranton branch of Dunder Mifflin, a distribution company dealing in paper products that is currently under threat of facing downsizing.");
    ...
    public void actorOPeliEstaNominado() {
        for (INominable nombre : nominados) {
            if (nominados.contains(nombre))
                nombre.estaNominada();

        }
        System.out.println("no esta nominado");
    }
